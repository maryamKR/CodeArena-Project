...
// Fallback mock data (used until API responds)
const DEFAULT_CATEGORIES = [
  { id: 'js', label: 'JavaScript', short: 'JS', count: 142, solved: 0, color: '#e6db74' },
  { id: 'py', label: 'Python', short: 'PY', count: 98, solved: 0, color: '#66d9e8' },
  { id: 'sql', label: 'SQL', short: 'SQL', count: 76, solved: 0, color: '#f92672' },
  { id: 'algo', label: 'Algorithms', short: 'AL', count: 54, solved: 0, color: '#a6e22e' },
];
...
export default function Home() {
  ...
  const [topPlayers, setTopPlayers] = useState(DEFAULT_TOP_PLAYERS);
  const [recentActivity, setRecentActivity] = useState(DEFAULT_RECENT_ACTIVITY);
  const [dailyChallenge, setDailyChallenge] = useState(null);
  const [myRank, setMyRank] = useState(null);

  // Real user stats (from user object in AuthContext)
  const USER_STATS = [
  ...
  useEffect(() => {
    ...
    // Fetch daily challenge (only if logged in — auth required)
    if (user) {
      api.get('/daily-challenge')
        .then(res => setDailyChallenge(res.data.data))
        .catch(() => {});
  ...
  // Format resetsIn seconds to HH:MM:SS
  const formatResetsIn = (seconds) => {
    if (!seconds) return '--:--:--';
          ...
          {dailyChallenge ? (
            <>
              ...
              <div style={styles.dcTags}>
                <span style={{ ...styles.dcPill, color: '#e6db74', borderColor: '#e6db74' }}>
                  {dailyChallenge.category?.slug?.toUpperCase() || 'JS'}
                </span>
              ...
              <div style={styles.dcFooter}>
                <span style={styles.dcTimer}>
                  ...
                  resets in {formatResetsIn(dailyChallenge.resetsIn)}
                </span>
                <button
            ...
            <div style={{ color: '#75715e', fontSize: '13px', marginTop: '12px' }}>
              {user ? '// no_daily_challenge_today' : '// login_to_see_daily_challenge'}
            </div>
          )}
          {!user && (
          ...
          <div style={{ marginTop: '10px' }}>
            {topPlayers.length > 0 ? topPlayers.map((p, i) => (
              ...
              >
                ...
                <span style={{ color: '#75715e' }}>{p.totalXP} XP</span>
              </div>
            )) : (
              <div style={{ color: '#75715e', fontSize: '12px' }}>// loading...</div>
            )}
            {user && myRank && (
              <div style={{ ...styles.lbRow, color: '#66d9e8', paddingTop: '8px' }}>
                <span>#{myRank.globalRank} you</span>
                <span>{myRank.totalXP} XP</span>
              </div>
            )}
          </div>
            ...
            <div style={styles.activityGrid}>
              {recentActivity.length > 0 ? recentActivity.map((a) => (
                ...
                <div style={{ color: '#75715e', fontSize: '12px', gridColumn: '1/-1' }}>
                  // no_activity_yet — play a quiz to see your history!
                </div>
              )}
            </div>
          </div>
...
const styles = {
  ...
  panelsGrid: { display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px', padding: '16px 24px 0' },
  ...
  dcLogin: { fontFamily: "'Space Mono', monospace", fontSize: '11px', fontWeight: 700, color: '#66d9e8', border: '2px solid #66d9e8', padding: '6px 14px', background: 'transparent', cursor: 'pointer', marginTop: '12px', display: 'inline-block' },
