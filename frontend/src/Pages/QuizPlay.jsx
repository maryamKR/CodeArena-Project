...
export default function Quiz() {
    ...
    const [focusMode, setFocusMode] = useState(() => localStorage.getItem('ca_focus_mode') === 'true');
    ...
    const handleNext = useCallback(async () => {
        if (current + 1 >= questions.length) {
            try {
                const res = await api.post('/scores', { answers, difficulty, timeLeft: timer, timeLimit: TIMER_MAX * questions.length, categoryId, isDailyChallenge });
                await refreshUser();
                navigate('/results', { state: { result: res.data.data, score, total: questions.length, category, difficulty, review } });
            } catch {
                navigate('/results', { state: { score, total: questions.length, category, difficulty, review } });
            }
            return;
        }
        setCurrent(c => c + 1); setSelected(null); setAnswered(false); setResult(null); setTimer(TIMER_MAX);
    ...
    const handleAnswer = useCallback(async (answer) => {
        if (answered) return;
        setAnswered(true); setSelected(answer);
        setAnswers(prev => [...prev, { questionId: questions[current]._id, selectedAnswer: answer }]);
        try {
            const res = await api.post(`/questions/${questions[current]._id}/check`, { selectedAnswer: answer });
            const correctAnswer = res.data.correctAnswer;
            setQuestions(prev => prev.map((q, i) => i === current ? { ...q, correct_answer: correctAnswer } : q));
            setReview(prev => [...prev, { text: questions[current].text, selected: answer, correct: correctAnswer, isCorrect: res.data.correct }]);
            if (res.data.correct) { setResult('correct'); setScore(s => s + 1); setXpGain(10); setTimeout(() => setXpGain(null), 1500); }
            else { setResult('wrong'); }
        } catch { setResult('wrong'); }
    }, [answered, questions, current]);

    const handleForfeit = () => navigate('/dashboard');
    ...
    useEffect(() => {
        ...
        if (timer === 0) {
            setTimeout(() => { setExploding(true); setTimeout(() => { setExploding(false); handleNext(); }, 1000); }, 0);
            return;
        }
    ...
    return (
        <div style={{ ...styles.page, background: t.pageBg, animation: exploding ? 'shake 0.5s' : 'none' }}>
                    ...
                    {!isMobile && (
                        <div style={styles.navLinks}>
                            {NAV_LINKS.map((link, i) => (
                                <a key={link.label} onClick={() => navigate(link.path)} style={{ ...styles.navLink, borderColor: t.border, color: t.textMuted, ...(i === 2 ? styles.navLinkActive : {}), ...(i === NAV_LINKS.length - 1 ? { borderRight: `2px solid ${t.border}` } : { borderRight: 'none' }), cursor: 'pointer' }}>
                                    {link.label}
                                </a>
                            ))}
                        </div>
            ...
            <div style={{ ...(focusMode ? styles.contentFocus : styles.content), padding: isMobile ? '20px 16px' : (focusMode ? '48px 24px' : '28px 24px') }}>

                {/* Header row */}
                <div style={{ ...styles.headerRow, flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? '10px' : '0', alignItems: isMobile ? 'flex-start' : 'center' }}>
                ...
                <div style={styles.timerRow}>
                    <div style={{ ...styles.timerCircle, borderColor: getTimerColor(), color: getTimerColor(), boxShadow: `0 0 20px ${getTimerColor()}40`, width: isMobile ? '64px' : '80px', height: isMobile ? '64px' : '80px' }}>
                        <div style={{ ...styles.timerNum, fontSize: isMobile ? '20px' : '24px' }}>{timer}</div>
                ...
                {/* Question */}
                <div style={{ ...styles.questionCard, background: t.cardBg, borderColor: t.border, boxShadow: t.shadow, ...(focusMode ? styles.questionCardFocus : {}), padding: isMobile ? '18px' : (focusMode ? '48px' : '24px') }}>
                    <div style={{ ...styles.questionNum, color: t.textMuted }}>// question_{current + 1}</div>
                    <div style={{ ...styles.questionText, color: t.text, fontSize: isMobile ? '15px' : (focusMode ? '28px' : '18px'), ...(focusMode && !isMobile ? styles.questionTextFocus : {}) }}>{question?.text}</div>
                </div>

                {xpGain && <div style={{ ...styles.xpPop, color: t.green }}>+{xpGain} XP</div>}

                {/* True / False buttons */}
                <div style={{ ...styles.answerRow, gap: isMobile ? '10px' : '16px' }}>
                    <button
                        style={{
                            ...styles.answerBtn, ...styles.trueBtn, background: t.cardBg, boxShadow: t.shadow,
                            fontSize: isMobile ? '16px' : '20px', padding: isMobile ? '16px' : '24px',
                    ...
                    >
                        <span style={styles.answerKey}>[T / 1]</span>TRUE
                    </button>
                    <button
                        style={{
                            ...styles.answerBtn, ...styles.falseBtn, background: t.cardBg, boxShadow: t.shadow,
                            fontSize: isMobile ? '16px' : '20px', padding: isMobile ? '16px' : '24px',
                    ...
                    >
                        <span style={styles.answerKey}>[F / 2]</span>FALSE
                    </button>
                </div>

                {answered && (
                    <div style={{ ...styles.resultRow, flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? '6px' : '0' }}>
                        {result ? (
            ...
            {confirmForfeit && (
                <div style={styles.overlay}>
                    <div style={{ ...styles.dialog, background: t.cardBg, boxShadow: t.shadow, width: isMobile ? '95%' : '90%' }}>
                        ...
                        <div style={{ ...styles.dialogBtns, flexDirection: isMobile ? 'column' : 'row' }}>
                            <button style={styles.dialogCancel} onClick={() => setConfirmForfeit(false)}>← KEEP PLAYING</button>
                            <button style={styles.dialogConfirm} onClick={handleForfeit}>✕ QUIT</button>
                        </div>
                    </div>
                </div>
            )}
